...
async fn get_rwa_accounts(
    db: &DatabaseConnection,
    id: Vec<u8>,
) -> Result<Option<asset_controller::Model>, DbErr> {
    let account = asset_controller::Entity::find()
        .filter(asset_controller::Column::Authority.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}

async fn get_data_registry(
    db: &DatabaseConnection,
    id: Vec<u8>,
) -> Result<Option<data_registry::Model>, DbErr> {
    let account = data_registry::Entity::find()
        .filter(data_registry::Column::Authority.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}

async fn get_identity_registry(
    db: &DatabaseConnection,
    id: Vec<u8>,
) -> Result<Option<identity_registry::Model>, DbErr> {
    let account = identity_registry::Entity::find()
        .filter(identity_registry::Column::Authority.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}

async fn get_policy_engine(
    db: &DatabaseConnection,
    id: Vec<u8>,
) -> Result<Option<policy_engine::Model>, DbErr> {
    let account = policy_engine::Entity::find()
        .filter(policy_engine::Column::Authority.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}

pub async fn get_rwa_accounts_by_authority(
    db: &DatabaseConnection,
    id: Vec<u8>,
) -> Result<FullAccount, DbErr> {
    let get_accounts_controller_future = get_rwa_accounts(db, id.clone());
