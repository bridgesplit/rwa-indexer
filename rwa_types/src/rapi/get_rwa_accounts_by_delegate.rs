...
async fn get_rwa_accounts(
    ...
    let account = asset_controller::Entity::find()
        .filter(asset_controller::Column::Delegate.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}
...
async fn get_data_registry(
    ...
    let account = data_registry::Entity::find()
        .filter(data_registry::Column::Delegate.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}
...
async fn get_identity_registry(
    ...
    let account = identity_registry::Entity::find()
        .filter(identity_registry::Column::Delegate.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}
...
async fn get_policy_engine(
    ...
    let account = policy_engine::Entity::find()
        .filter(policy_engine::Column::Delegate.eq(id.clone()))
        .one(db)
        .await?;
    Ok(account)
}

pub async fn get_rwa_accounts_by_delegate(
    db: &DatabaseConnection,
    id: Vec<u8>,
